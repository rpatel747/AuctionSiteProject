...
<html>
	...
	<body>
		...
		try{
			...
			PreparedStatement getTotalSold = con.prepareStatement("SELECT SUM(soldPrice) FROM sells WHERE saleStatus=1");
			ResultSet totalSalesValue = getTotalSold.executeQuery();
			
			PreparedStatement getTotalNumberOfSold = con.prepareStatement("SELECT COUNT(soldPrice) FROM sells WHERE saleStatus=1");
			...
			<table id="viewSalesTable">
				<tr>
					...
					<th>Total Amount Made From Sales</th>
				</tr>
				<tr>
					<td><%=totalEarnings %></td>
			...
			PreparedStatement getEarningsForTrucks = con.prepareStatement("SELECT SUM(soldPrice) FROM sells WHERE saleStatus=1 AND saleNumber IN(SELECT saleNumber FROM sale WHERE vehicleType=?)");
			getEarningsForTrucks.setString(1,"truck");
			ResultSet trucksEarnings = getEarningsForTrucks.executeQuery();
			
			PreparedStatement getNumberOfTrucksSold = con.prepareStatement("SELECT COUNT(*) FROM sells WHERE saleStatus=1 AND saleNumber IN(SELECT saleNumber FROM sale WHERE vehicleType=?)");
			getNumberOfTrucksSold.setString(1,"truck");
			ResultSet numOfTrucks = getNumberOfTrucksSold.executeQuery();
			
			PreparedStatement getEarningsForCars = con.prepareStatement("SELECT SUM(soldPrice) FROM sells WHERE saleStatus=1 AND saleNumber IN(SELECT saleNumber FROM sale WHERE vehicleType=?)");
			getEarningsForCars.setString(1,"car");
			ResultSet carsEarnings = getEarningsForCars.executeQuery();
			
			PreparedStatement getNumberOfCarsSold = con.prepareStatement("SELECT COUNT(*) FROM sells WHERE saleStatus=1 AND saleNumber IN(SELECT saleNumber FROM sale WHERE vehicleType=?)");
			getNumberOfCarsSold.setString(1,"car");
			ResultSet numOfCars = getNumberOfCarsSold.executeQuery();
			
			PreparedStatement getEarningsForMotorbikes = con.prepareStatement("SELECT SUM(soldPrice) FROM sells WHERE saleStatus=1 AND saleNumber IN(SELECT saleNumber FROM sale WHERE vehicleType=?)");
			getEarningsForMotorbikes.setString(1,"motorbike");
			ResultSet motorbikesEarnings = getEarningsForMotorbikes.executeQuery();
			
			PreparedStatement getNumberOfMotorbikesSold = con.prepareStatement("SELECT COUNT(*) FROM sells WHERE saleStatus=1 AND saleNumber IN(SELECT saleNumber FROM sale WHERE vehicleType=?)");
			...
			if(customerUsernameAndEmails.next()){
				do{
					...
					PreparedStatement getSalesHistory = con.prepareStatement("SELECT COUNT(soldPrice),SUM(soldPrice) FROM sells WHERE email=? AND saleStatus = 1");
					getSalesHistory.setString(1,customerEmail);
					ResultSet salesHistory = getSalesHistory.executeQuery();
					
					PreparedStatement getBidsHistory = con.prepareStatement("SELECT COUNT(bidPrice),SUM(bidPrice) FROM won WHERE email=?");
					...
					if(salesHistory.next()){
						...
						if(!c2.equals("null")){
							earningsFromSelling = Integer.parseInt((String.valueOf(salesHistory.getObject(2))));
						}
					...
					if(bidsHistory.next()){
						...
						if(!c2.equals("null")){
							earningsFromBuying = Integer.parseInt((String.valueOf(bidsHistory.getObject(2))));
						}
			...
			PreparedStatement getBestSellingItems = con.prepareStatement("SELECT carName,manufacturer,vehicleType,COUNT(carName) FROM sale INNER JOIN won ON sale.saleNumber=won.saleNumber WHERE status=1 GROUP BY carName,manufacturer,vehicleType ");
			ResultSet bestSellingItems = getBestSellingItems.executeQuery();
			
			
			
			String[][] carDetails = new String[4][500];
			...
			PreparedStatement getBestBuyers = con.prepareStatement("SELECT won.email,SUM(bidPrice),COUNT(bidPrice) FROM won INNER JOIN customerHas ON won.email=customerHas.email GROUP BY email");
			...
			if(bestBuyers.next()){
				...
				do{
					...
					String buyerEmail = String.valueOf(bestBuyers.getObject(1));
					...
					if(firstMostSold == -1 || Integer.valueOf(buyerTotalAmount) >= Integer.valueOf(buyerDetails[2][firstMostSold]) ){
					...
					else if(secondMostSold == -1 || Integer.valueOf(buyerTotalAmount) >= Integer.valueOf(buyerDetails[2][secondMostSold])){
						int oldSecond = secondMostSold;
					...
					else if(thirdMostSold == -1  || Integer.valueOf(buyerTotalAmount) >= Integer.valueOf(buyerDetails[2][thirdMostSold])){
						fourthMostSold = k;
						k++;
					}
